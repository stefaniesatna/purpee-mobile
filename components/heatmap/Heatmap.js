...
export const Heatmap = ({ handleSquarePress }) => {
  ...
  const getNumOfRows = (screenRatio) => {
    ...
    if (
      screenRatio < iPadSmall.ratioThreshold &&
      screenHeight < iPadSmall.heightThreshold
    )
      return iPadSmall.numOfRows;
    if (
      screenRatio < iPadBig.ratioThreshold &&
      screenHeight < iPadBig.heightThreshold
    )
      return iPadBig.numOfRows;
    else return 8;
  };
